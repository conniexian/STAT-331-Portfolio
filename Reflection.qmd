...
teacher_evals_clean |> 
  ...
  filter(avg_percent_failed == max(avg_percent_failed) | 
         avg_percent_failed == min(avg_percent_failed)) |>
  arrange(avg_percent_failed) |> 
  kable()
```
...
# Revision Reflection: Initially, I used an | operator to filter the data. However, by using the %in% operator, I can increase the efficiency of my code. Using the %in% operator also makes my code more easily readable, since it is more clear how the data is being filtered now.

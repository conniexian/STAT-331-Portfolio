...
get_fit_now_member |>
  ...
  # Find all gold members with a membership number that start with 48Z
  filter(membership_status == "gold",              
         str_sub(id, start = 1, end = 3) == "48Z") |>
  
  # Use person data to find associated license
  left_join(person,
            join_by(person_id == id, name)) |>         
  
  # Use license data to find suspect with plate number that includes "H42"
  left_join(drivers_license,                           
            join_by(license_id == id))  |>             
  filter(str_detect(plate_number,
                    pattern = "H42W")) |> 
  
  # Check that date matches statement from second witness 

...
# Lab 5 - The Witnesses (Revised for Portfolio)
# Searching for Witness 1
witness1_address <- tibble(street = "Northwestern Dr")
person |> 
  semi_join(witness1_address,                          # Find first witness
            by = join_by(address_street_name == street)) |>  
  slice_max(address_number) |> 
  left_join(y = interview,                             # Search for interview by person_id
            by = join_by(id == person_id)) |>          
  select(transcript)  |>                               # View interview transcript
...
# Revision Reflection: Initially, I had mutated each column individually
# as I was unsure whether I wanted to mutate them all to the same 
# function type. However, I should have realized, after I set them all
# to factors, that I could have converted them all at once using an 
# across() function. Doing so would be more efficient since the function
